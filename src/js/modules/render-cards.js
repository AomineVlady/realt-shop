...
import { cardsList, cardsWrapper, priceTransform, dateTransform, clearHTMLItem, getCardContentData, COUNT_CARDS} from './common.js'
...
const getCardsElements = (card) => {
  return `<li class="results__item product" id = "${card.id}">
        <button class="product__favourite fav-add${card.favorite ? " fav-add--active" : ""}" type="button" aria-label="Добавить в избранное" id = "card_${card.id}">
        ...
        <div class="product__content">
          ...
          <div class="product__date">${dateTransform(card['publish-date'])}</div>
        </div>
      </li>`;
};
